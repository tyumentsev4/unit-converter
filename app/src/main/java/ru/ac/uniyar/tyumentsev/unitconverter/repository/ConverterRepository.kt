...
class ConverterRepository(private val converterDao: ConverterDao) {
    suspend fun insertConverter(converter: ConverterEntity) =
        converterDao.insertConverter(converter)
    ...
    suspend fun getUnitsForConverter(converterId: Long): List<UnitEntity> =
        converterDao.getUnitsForConverter(converterId)

    suspend fun removeConverter(converter: ConverterEntity) =
        converterDao.removeConverter(converter)
}
